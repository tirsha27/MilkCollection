...
//     fullnessPercent: number;
//     lowUtilized: { vehicle_number: string; pct: number }[];
//     unusedVehicles?: string[]; // NEW: list of unused vehicle_numbers
//   };
// }
...
//    *      - vehicle_number (replacing `type` display when possible)
//    *      - per-cluster stats (vehiclesUsed, totalAssignedMilk, fullnessPercent, lowUtilized list)
//    *      - converts v.route (string[]) => farmers[] when needed
//    */
//   const loadSchedule = async () => {
//     try {
//       setLoading(true);
...
//         const vehicles: Trip[] =
//           (cluster.vehicles || []).map((v: any, vIdx: number) => {
//             // prefer explicit vehicle_number from API; fall back to fleetMap[type] or typeKey
//             const typeKey = v.type ?? v.vehicle_type ?? v.category_name ?? v.category ?? `Vehicle ${vIdx + 1}`;
//             const vehicleNumberFromAPI = v.vehicle_number ?? v.vehicleNumber ?? v.vehicle_no ?? null;
//             const matchedVehicleNumber = vehicleNumberFromAPI ?? fleetMap[String(typeKey)] ?? String(typeKey);

//             // BUILD FARMERS:
//             // - if API already provides farmers array (objects), use it
//             // - else if API provides route: string[] -> convert to Farmer[]
//             let farmers: Farmer[] = [];
//             if (Array.isArray(v.farmers) && v.farmers.length > 0) {
//               farmers = (v.farmers || []).map((f: any, fIdx: number) => {
//                 const lat = f?.lat ?? 10.0 + Math.random() * 0.5;
//                 const lng = f?.lng ?? 78.0 + Math.random() * 0.5;
//                 return {
//                   id: `${cluster.name}-v${vIdx}-f${fIdx}`,
...
//               // v.route is array of farmer names (strings). Distribute total_milk evenly across stops.
//               const names: string[] = v.route.map((r: any) => String(r));
//               const totalMilkNum = Number(v.total_milk ?? v.totalMilk ?? 0) || 0;
//               const per = names.length > 0 ? Math.floor(totalMilkNum / names.length) : 0;
//               let remainder = totalMilkNum - per * names.length;
//               farmers = names.map((nm, fIdx) => {
...
//             const totalMilk = v.total_milk ?? v.totalMilk ?? farmers.reduce((sum: number, f: Farmer) => sum + (f?.milk_liters ?? 0), 0);

//             const capacityVal = v?.capacity ?? v?.capacity_liters ?? 0;

//             const utilizationVal = Number(
//               v.utilization ??
//                 v.utilization_pct ??
//                 (capacityVal ? Math.round((Number(totalMilk) / (capacityVal || 1)) * 100) : 0)
//             ) || 0;

//             // If this vehicle has no stops (farmers) and no milk — mark metrics as zero to avoid stale values
//             const stops = farmers.length;
//             const travelTime = stops === 0 ? 0 : (v.travel_time ?? Math.floor(Math.random() * 120) + 60);
//             const distanceVal = stops === 0 ? 0 : (v.distance ?? Math.floor(Math.random() * 50) + 20);
//             const utilizationFinal = stops === 0 ? 0 : utilizationVal;

//             return {
//               // keep droppable id used by drag/drop logic stable
//               id: `droppable-${cIdx}-${vIdx}`,
//               type: String(typeKey),
//               vehicle_number: matchedVehicleNumber,
//               farmers,
//               total_milk: totalMilk,
//               capacity: capacityVal,
//               // if there are no stops set distance/time to 0 to avoid stale display
//               distance: distanceVal,
//               travel_time: travelTime,
//               utilization: utilizationFinal,
//             } as Trip;
//           }) || [];
...
//         const centerCapacity = Number(cluster.capacity ?? cluster.capacity_liters ?? cluster.total_capacity ?? 0);
//         const fullnessPercent = centerCapacity > 0 ? (totalAssignedMilk / centerCapacity) * 100 : 0;
//         // exclude empty vehicles (no farmers) from lowUtilized warnings
//         const lowUtilized = vehicles
//           .filter((vv) => {
...
//         // NEW: list unused vehicle_numbers for cluster header
...
//             fullnessPercent: Math.round((fullnessPercent + Number.EPSILON) * 100) / 100,
//             lowUtilized,
//             unusedVehicles, // attach unused list
//           },
//         } as CenterSchedule;
//       });
...
//     // parse indices from droppableId format 'droppable-{clusterIdx}-{vehicleIdx}'
...
//     // capture previous counts & travel_time for proportional estimation
...
//     // recompute totals for both vehicles
//     const recomputeForVehicle = (veh: Trip, prevStops: number, prevTravel: number, prevDistance: number) => {
//       // total_milk
//       veh.total_milk = (veh.farmers ?? []).reduce((sum, f) => sum + (f.milk_liters ?? 0), 0);

//       // new stops count
//       const newStops = (veh.farmers ?? []).length;

//       // If no stops => mark as unused explicitly (zero metrics)
//       if (newStops === 0) {
//         veh.total_milk = 0;
//         veh.utilization = 0;
//         veh.travel_time = 0;
...
//       // utilization: prefer capacity field, avoid division by zero
//       const cap = Number(veh.capacity ?? 0);
//       if (cap > 0) {
//         veh.utilization = Math.round((Number(veh.total_milk || 0) / cap) * 10000) / 100; // 2 decimal precision
//       } else {
//         // fallback: if capacity missing, set utilization to 100 if there is milk
//         veh.utilization = veh.total_milk > 0 ? 100 : 0;
//       }

//       // travel_time estimation:
//       // if previous travel_time exists and prevStops > 0, scale proportionally to number of stops
//       if (prevStops > 0 && prevTravel > 0) {
//         veh.travel_time = Math.max(1, Math.round((prevTravel * newStops) / prevStops));
//       } else {
//         // fallback heuristic: assume ~8-12 minutes per stop + base 10 minutes
//         const perStop = 10;
//         veh.travel_time = Math.round(10 + newStops * perStop);
//       }

//       // distance estimation: scale proportionally if we had a previous distance else heuristic (5-8 km per stop)
...
//       // NEW: compute unused vehicles list
//       const unusedVehicles = vehicles
//         .filter((vv) => (vv.farmers ?? []).length === 0)
//         .map((vv) => vv.vehicle_number ?? vv.type ?? "Unknown");
...
//                   <h3 className="font-bold text-xl text-slate-900">{cluster.name}</h3>
//                   <span className="text-slate-600 ml-2">
//                     Chilling Centre :{cluster.capacity}L total
//                   </span>
...
//                       </div>
//                     )}
//                     {/* NEW: show unused vehicles badge in cluster header */}
...
//                   {cluster.vehicles.map((vehicle) => {
//                     const isTripExpanded = expandedTrips.has(vehicle.id);
//                     // ensure vehicle.farmers is an array (we normalized above)
...
//                           >
//                             {/* -------------------------
//                                 Compact message under vehicle number (header)
//                                 ------------------------- */}
//                             <div
...
//                               }}
//                             >
//                               {/* LEFT: vehicle id + (message beneath) + stops */}
//                               <div className="flex items-start gap-3">
//                                 <Truck className="text-green-600" />

//                                 {/* make the title+message a column so message sits right under the vehicle number */}
...
//                                     {/* stops count (kept inline) */}
//                                     <span className="text-slate-600 text-sm ml-3">
//                                       {stopsCount} stops
//                                     </span>
//                                      {typeof vehicle.capacity !== "undefined" && vehicle.capacity !== null && (
//                                       <span className="text-sm text-slate-500 ml-2">
//                                         Total Vehicle Capacity {Number(vehicle.capacity).toLocaleString()} L
//                                       </span>
//                                     )}
//                                   </div>

//                                   {/* Inline LOW-UTIL message directly under vehicle number — small and readable */}
...
//                                         <strong className="text-sm">Low Utilization:</strong>{" "}
//                                         This vehicle is only <strong>{vehicle.utilization}%</strong> full.
//                                         Consider using a smaller vehicle (Auto/Jeeto) for better efficiency.
//                                       </div>
//                                     </div>
//                                   )}

//                                   {/* NEW: Unused flag — show when all farmers moved away */}
//                                   {isUnused && (
...
//                               {/* RIGHT: metrics (distance/time/milk/util)
//                                   If vehicle is unused we show 0 values (no stale numbers) */}
//                               <div className="flex items-center gap-4 text-slate-600 text-sm">
//                                 <Ruler className="h-4 w-4 text-blue-500" /> Distance to complete : {isUnused ? 0 : vehicle.distance} km
//                                 <Timer className="h-4 w-4 text-orange-500" />Time Taken to complete : {isUnused ? 0 : vehicle.travel_time} mins
...
/**
 * Allocate milk to farmers such that:
 * - If declaredTotal > 0: allocations (integers) sum exactly to declaredTotal.
 * - Known milk_liters from rawFarmers are preserved. Unknowns get allocated via random-weighted split.
 * - If declaredTotal === 0 and no knowns: fallback random allocation.
 */
function allocateMilkToFarmers(
  ...
  // No declared total & no knowns -> random fallback
  if ((declaredTotal ?? 0) <= 0 && knownSum <= 0) {
    return rawFarmers.map((rf) => ({
      id: rf.id,
      name: rf.name,
      milk_liters: Math.floor(Math.random() * 31) + 10, // 10..40
      lat: rf.lat,
      lng: rf.lng,
  ...
  if ((declaredTotal ?? 0) > 0) {
    ...
    if (unknownCount > 0) {
      const weights = unknowns.map(() => Math.random() + 0.01);
      const wsum = weights.reduce((s, w) => s + w, 0);
      const floatAlloc = weights.map((w) => (w / wsum) * remaining);

      const floored = floatAlloc.map((v) => Math.floor(v));
      let flooredSum = floored.reduce((s, n) => s + n, 0);
      let remainder = remaining - flooredSum;

      const fractions = floatAlloc.map((v, idx) => ({ idx, frac: v - Math.floor(v) }));
      fractions.sort((a, b) => b.frac - a.frac);

      const addOne = new Array(unknownCount).fill(0);
      for (let i = 0; i < remainder; i++) {
        addOne[fractions[i % unknownCount].idx] += 1;
      }

      unknowns.forEach((u, i) => {
        const amt = floored[i] + addOne[i];
        allocations.push({
          id: u.id,
          name: u.name,
          milk_liters: amt > 0 ? amt : 1,
          lat: u.lat,
          lng: u.lng,
          location: u.location,
        });
    ...
    // final correction
    const sumNow = allocations.reduce((s, f) => s + (Number(f.milk_liters) || 0), 0);
    const diff = totalToDistribute - sumNow;
    if (allocations.length > 0 && diff !== 0) {
      allocations[0].milk_liters = Math.max(0, Number(allocations[0].milk_liters || 0) + diff);
    }

    return allocations;
  }

  // No declared total but knownSum > 0: preserve knowns and give reasonable randoms to unknowns
  const allocations: Farmer[] = [];
  for (const rf of rawFarmers) {
    if (rf.milk_liters != null) {
      ...
      allocations.push({
        id: rf.id,
        name: rf.name,
        milk_liters: Math.floor(Math.random() * 21) + 5,
        lat: rf.lat,
        lng: rf.lng,
        location: rf.location,
      });
...
export default function TripSchedule() {
  ...
  // ensure unique farmer ids across whole run
  ...
  const loadSchedule = async () => {
    try {
      ...
      // fetch fleet map (category->vehicle_number)
      ...
      try {
        ...
        for (const f of fleetArr) {
          const cat = f?.category_name ?? null;
          const vn = f?.vehicle_number ?? f?.vehicle_no ?? f?.vehicleNumber ?? null;
          if (cat && vn && !fleetMap[cat]) fleetMap[cat] = vn;
        }
      } catch (err) {
        console.warn("Failed to fetch fleet list for mapping:", err);
      }

      // seen set: keep the first occurrence, skip duplicates
      ...
      const normalized = clusters.map((cluster: any, cIdx: number) => {
        ...
        for (let origIdx = 0; origIdx < vehiclesRaw.length; origIdx++) {
          ...
          // resolve candidate vehicle name
          const typeKey = v.type ?? v.vehicle_type ?? v.category_name ?? v.category ?? `Vehicle ${origIdx + 1}`;
          const candidate = String(v.vehicle_number ?? v.vehicleNumber ?? v.vehicle_no ?? fleetMap[String(typeKey)] ?? String(typeKey));

          // If we've already seen this vehicle (exact string) skip it entirely
          if (seenVehicleNumbers.has(candidate)) {
            // skip duplicate vehicle (do not include)
            continue;
          }
          seenVehicleNumbers.add(candidate);
          ...
          if (Array.isArray(v.farmers) && v.farmers.length > 0) {
            rawFarmers = (v.farmers || []).map((f: any, fIdx: number) => {
              ...
              return { id, name: typeof f === "string" ? String(f) : f?.name ?? `Farmer ${fIdx + 1}`, milk_liters: milk, lat, lng, location: generateLocation(lat, lng) };
            });
          } else if (Array.isArray(v.route) && v.route.length > 0) {
          ...
          const stops = farmers.length;
          const travelTimeVal = stops === 0 ? 0 : Number(v.travel_time ?? Math.floor(Math.random() * 120) + 60);
          const distanceVal = stops === 0 ? 0 : Number(v.distance ?? Math.round((Math.random() * 50 + 20) * 100) / 100);
  ...
  const handleDragEnd = (result: DropResult) => {
    ...
    const source = result.source;
    const destination = result.destination;
    const updatedClusters = JSON.parse(JSON.stringify(data.clusters)) as CenterSchedule[]; // deep clone for safe mutation

    // parse indices from droppableId 'droppable-{clusterIdx}-{vehicleIdx}'
    const [srcClusterIdx, srcVehicleIdx] = source.droppableId
      .replace("droppable-", "")
      .split("-")
      .map(Number);
                              ...
                              <div className="p-4 border-t border-slate-200 bg-slate-50 grid grid-cols-2 gap-4">
                                ...
                                <div className="h-[600px] rounded-lg overflow-hidden border border-slate-200 shadow-lg">
                                  <MapContainer center={routeCoords.length ? routeCoords[0] : [10.5, 78.1]} zoom={10} style={{ height: "100%", width: "100%" }}>
                                    ...
                                    {nearestCenter && (
                                      <Marker position={[nearestCenter.lat || 0, nearestCenter.lng || 0]} icon={L.divIcon({
                                        html: `<div style='background-color:#16a34a;color:white;border-radius:50%;font-size:10px;width:26px;height:26px;display:flex;align-items:center;justify-content:center;font-weight:700;'>🏭</div>`,
                                        className: "hub-icon",
                                      })}>
                                        <Popup>
