...
class OptimizationService:
    ...
    async def load_data_from_database(self):
        ...
        # Load active storage hubs (chilling centers)
    ...
    async def get_vehicle_types_from_db(self) -> List[Dict]:
        ...
        for vehicle in vehicles:
            ...
            if category not in vehicle_categories:
                # Extract specs from realistic_specs JSON if available
                specs = vehicle.realistic_specs or {}
                
                vehicle_categories[category] = {
                    'id': len(vehicle_categories) + 1,
                    'name': f"{category.capitalize()}_Van",
                    'capacity': vehicle.capacity_liters,
                    'count': 0,
                    'service_time': specs.get('service_time', 10),  # Default 10 min
                    'cost_per_km': specs.get('cost_per_km', 8),     # Default ₹8/km
                    'fixed_cost': specs.get('fixed_cost', 500)      # Default ₹500
                }
    ...
    def assign_heterogeneous_fleet(self, farmer_list: List[str], 
                                   cluster_name: str, 
                                   fleet_types_dict: List[Dict]) -> List[Dict]:
    ...
    async def run_optimization(self, deadline_minutes: int, 
        ...
        try:
            # Create distance matrix using OpenRouteService
            ...
            # Assign subareas to nearest chilling centers
            ...
            for centroid_name, subarea_list in cluster_assignments.items():
                ...
                # Assign vehicles to farmers in this cluster
                vehicle_assignments = self.assign_heterogeneous_fleet(
                    subarea_list, centroid_name, vehicle_types_list
                )
                ...
                # Process each vehicle assignment
