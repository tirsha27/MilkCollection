...
class StorageHubService:
    ...
    # -------------------------------------------------------------------------
    @staticmethod
    async def create_hub(db: AsyncSession, hub_data: StorageHubCreate) -> StorageHub:
        """Create a single storage hub"""
        result = await db.execute(
            select(StorageHub).order_by(desc(StorageHub.hub_code)).limit(1)
        )
        last_hub = result.scalar_one_or_none()
        last_num = 0
        ...
        new_code = generate_storage_hub_code(last_num + 1)
        ...
        new_hub = StorageHub(
            ...
            contact_number=getattr(hub_data, "contact_number", None),
            capacity_liters=hub_data.capacity_liters,
            capacity_cans=liters_to_cans(hub_data.capacity_liters),
            current_load_liters=getattr(hub_data, "current_load_liters", 0.0) or 0.0,
            current_load_cans=liters_to_cans(getattr(hub_data, "current_load_liters", 0.0) or 0.0),
            is_active=True
        )
